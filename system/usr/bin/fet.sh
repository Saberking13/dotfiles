...
if [ -e /proc/$$/comm ]; then
	...
	# then save the argument count to $emerge
	set --
	...
	Open*)
		...
		[ -d /var/db/pkg ] && set -- /var/db/pkg/* && emerge=$#
...
# default value
: "${info:=n os up mem kern emerge infi col n}"

custommem=$(free -m | grep Mem | awk '{print $2}')
customswap=$(free -m | grep Swap | awk '{print $2}')
customtotal=$(($custommem + customswap))
customusedmem=$(free -m | grep Mem | awk '{print $3}')
customusedswap=$(free -m | grep Swap | awk '{print $3}')
customusedtotal=$(($customusedmem + $customusedswap))
cmem=$(printf "$customusedtotal""M / ""$customtotal""M")

for i in $info; do
	case $i in
		n) echo;;
		os) print "os+wm" "$PRETTY_NAME + $(echo $wm | sed 's/.*/\u&/')";;
		sh) print sh "${SHELL##*/}";;
		wm) print wm "$wm";;
		up) print uptime "$up";;
		gtk) print gtk "${gtk# }";;
		cpu) print cpu "$vendor$cpu";;
		mem) print memory "$cmem";;
		host) print host "$model";;
		kern) print kernel "$kernel";;
		emerge) print emerge "$emerge";;
		pacman) print pacman "$pacman";;
		infi) print infi "$infi";;
